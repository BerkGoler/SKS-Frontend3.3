...
import { createShipment, getShipment, updateShipment, listShipments } from "../services/ForwardService";
...
const ForwardComponent = () => {
    ...
    useEffect(() => {
        ...
        if (id) {
            getShipment(id).then(response => {
                ...
                console.log("Shipment data received:", shipment); // Log entire shipment object
                ...
                console.error("Error fetching shipment:", error); // Log any error in fetching shipment
            });
        }
    }, [id]);
    ...
    const fetchDropdownOptions = () => {
        listShipments().then(response => {
            ...
            const uniqueCompanyNames = [...new Set(shipments.map(item => item.company.companyName))];
            const uniqueProductNames = [...new Set(shipments.map(item => item.product.productName))];
            const uniqueShipNames = [...new Set(shipments.map(item => item.ship.shipName))];
            setCompanyNameOptions(uniqueCompanyNames);
            setProductNameOptions(uniqueProductNames);
            setShipNameOptions(uniqueShipNames);
        }).catch(error => {
                            ...
                            <form>
                                <div className='form-group mb-2'>
                                    ...
                                    >
                                        ...
                                        {companyNameOptions.map((name, index) => (
                                ...
                                <div className='form-group mb-2'>
                                    ...
                                    >
                                        ...
                                        {shipNameOptions.map((name, index) => (
                                            <option key={index} value={name}>{name}</option>
                                        ))}
                                    </select>
